...
[**下载 macOS 版**](https://github.com/ykswang/LLMHubX/releases/download/app-v20260909.0046-2/LLMHubX_20260909.0046-2_aarch64.dmg) · [查看更新说明](https://github.com/ykswang/LLMHubX/releases/tag/app-v20260909.0046-2) · [Marketplace 索引](./marketplace.toml)

![macOS](https://img.shields.io/badge/macOS-Apple%20Silicon-111827?style=flat-square&logo=apple&logoColor=white)
![App](https://img.shields.io/badge/App-20260909.0046--2-2563eb?style=flat-square)
...
1. 下载最新的 [Apple Silicon DMG](https://github.com/ykswang/LLMHubX/releases/download/app-v20260909.0046-2/LLMHubX_20260909.0046-2_aarch64.dmg)。
...
| 安装包 | [LLMHubX_20260909.0046-2_aarch64.dmg](https://github.com/ykswang/LLMHubX/releases/download/app-v20260909.0046-2/LLMHubX_20260909.0046-2_aarch64.dmg) |
| 更新说明 | [GitHub Release](https://github.com/ykswang/LLMHubX/releases/tag/app-v20260909.0046-2) |
| SHA-256 | [SHA256SUMS.txt](https://github.com/ykswang/LLMHubX/releases/download/app-v20260909.0046-2/SHA256SUMS.txt) |

下载后可在终端核对文件：

```bash
shasum -a 256 ~/Downloads/LLMHubX_20260909.0046-2_aarch64.dmg
```

将输出与 Release 中的 `SHA256SUMS.txt` 对比。

> [!IMPORTANT]
> 当前 `20260909.0046-2` 为 adhoc 签名构建，尚未使用 Apple Developer ID 签名或完成 notarization，macOS Gatekeeper 不会自动信任。安装前请先核对 SHA-256；首次启动如被系统阻止，请在“系统设置 → 隐私与安全”中确认 App 来源后再决定是否允许打开。

> [!WARNING]
> `20260831.1009` 和 `20260905.0836` 的已发布二进制无法解析自身内部 `.0` 版本；`20260907.0228-2`（界面显示 `20260907.228.2`）也存在前导零解析缺陷。上述版本请手动下载安装 `20260909.0046-2`。本版本不再把新密钥写入 macOS Keychain。首次读取旧配置时可选择一次性导入旧密钥；macOS 可能对每项旧密钥请求一次授权，导入完成后不再访问 Keychain。也可选择“重新配置”，完全跳过 Keychain，并在修改配置前保留备份。

### 本次更新

修复 Task 失败后的重规划参数错配，保留新运行的 Provider 与协调错误诊断；观测分别显示执行状态、步骤进度和通知送达。BotX 根据实际成果默认简短自然地回答，明确要求报告时再展开；回调失败保持待重试，升级不自动重跑旧任务。真实模型链路与回复体验待更新后完成最后验证。详情见[更新说明](https://github.com/ykswang/LLMHubX/releases/tag/app-v20260909.0046-2)。
